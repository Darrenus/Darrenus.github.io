...
const retrieve: ToolDef = {
  spec: {
    ...
    function: {
      name: "retrieve",
      description:
        "Search Allen's approved public material with keyword search (BM25). Returns ranked passages, each with a chunk id and document id. This is lexical, not semantic: search for terms likely to appear in the source and reformulate thin queries. Works in Chinese and English.",
      parameters: {
        type: "object",
        properties: {
          query: {
          ...
          index: {
            ...
            description:
              "Narrow the corpus. Use 'profile' for biography, education, experience, contact and achievements; use 'project' for projects. Omit to search everything.",
          },
          limit: {
            type: "integer",
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          doc_id: {
            type: "string",
            description: "Document id, e.g. about-about, about-experience, about-projects.",
          },
          section: {
            type: "string",
            description:
...
const webSearch: ToolDef = {
  spec: {
    ...
    function: {
      name: "web_search",
      description:
        "Search the live web for external current information. Do not use it to replace Allen's approved profile facts; the local index is authoritative for his background.",
      parameters: {
        type: "object",
        properties: {
...
const githubActivity: ToolDef = {
  spec: {
    ...
    function: {
      ...
      description:
        "List Allen's public, non-fork repositories from GitHub, ordered by recent pushes. Use it for current public coding activity.",
      parameters: {
        type: "object",
        properties: {},
        additionalProperties: false,
...
/** Resolve a repository name or owner/name within the approved account list. */
async function resolveRepo(
  ...
  if (parts.length >= 2) {
    ...
    if (!GITHUB_ACCOUNTS.includes(owner as (typeof GITHUB_ACCOUNTS)[number])) {
      return {
        error: `This tool only reads Allen's approved accounts (${GITHUB_ACCOUNTS.join(", ")}). "${owner}" is not one of them.`,
      };
    }
  ...
  const name = parts[0] ?? "";
  if (!name) return { error: "Which repository? Pass a repository name." };
  if (DENY_REPOS.has(name)) return { error: repoDenied(name) };
...
function repoDenied(name: string): string {
  return `"${name}" is not available to this public site.`;
}
...
const repoTree: ToolDef = {
  spec: {
    ...
    function: {
      ...
      description:
        "List files and directories in one of Allen's public repositories. Use it to find a path before calling github_read_file.",
      parameters: {
        type: "object",
        properties: {
          repo: {
            type: "string",
            description:
              "Repository name, or owner/name for exactness.",
          },
          path: {
            type: "string",
...
const readFile: ToolDef = {
  spec: {
    ...
    function: {
      ...
      description:
        "Read one file from one of Allen's public repositories. Quote only the relevant parts and name the path.",
      parameters: {
        type: "object",
        properties: {
...
const grepRepo: ToolDef = {
  spec: {
    ...
    function: {
      name: "github_grep",
      description:
        "Search one of Allen's public repositories for literal text and return matching files and line numbers. Use this before github_read_file when the path is unknown.",
      parameters: {
        type: "object",
        properties: {
          repo: { type: "string", description: "Repository name or owner/name." },
          pattern: {
            type: "string",
            description:
              "Literal text to find. Not a regular expression.",
          },
          path: {
            type: "string",
...
const readPull: ToolDef = {
  spec: {
    ...
    function: {
      name: "github_pull",
      description:
        "Read a pull request from one of Allen's public repositories, including its description, discussion and changed files.",
      parameters: {
        type: "object",
        properties: {
          repo: { type: "string", description: "Repository name or owner/name." },
